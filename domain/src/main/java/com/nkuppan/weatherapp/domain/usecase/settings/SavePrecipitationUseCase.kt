package com.nkuppan.weatherapp.domain.usecase.settings

import com.nkuppan.weatherapp.domain.model.Precipitation
import com.nkuppan.weatherapp.domain.model.Resource
import com.nkuppan.weatherapp.domain.respository.SettingsRepository

class SavePrecipitationUseCase(private val repository: SettingsRepository) {

    suspend operator fun invoke(precipitation: Precipitation): Resource<Boolean> {
        return Resource.Success(repository.savePrecipitation(precipitation))
    }
}